...
local _qm = QMQuestManager.CreateNew()
...
QMQuestManagerWindow.isOpen = false
...
function QMQuestManagerWindow.CreateNew(quest)
    if not quest then
        return nil
    end

    local instance = QMQuestManagerWindow.new{ quest = quest }
...
function QMQuestManagerWindow:_createWindow()
    ...
    windowRoot = gui.Panel {
        ...
        monitorGame = QMQuestManager.CreateNew():GetDocumentPath(),
    ...
    ThemeEngine.OnThemeChanged(mod, function()
        if windowRoot ~= nil and windowRoot.valid then
            windowRoot.styles = ThemeEngine.GetStyles()
        end
    end)
...
function QMQuestManagerWindow.CreateObjectivesPanel(quest)
    ...
    return gui.Panel {
        ...
        refreshObjectives = function(element)
            ...
            if scrollArea then
                scrollArea.children = reconcileObjectivesList(scrollArea.children)
            end
        end,
        children = {
...
function QMQuestManagerWindow.CreateNotesPanel(quest)
    local function reconcileNotesList(notePanels)
        ...
        for i, panel in ipairs(notePanels) do
            local isEven = (i % 2 == 0)
            panel:SetClass("evenRow", isEven)
            panel:SetClass("oddRow", not isEven)
        end
...
local CreateObjectiveDragHandle = function(quest, objective)
    return gui.Panel {
        classes = {"objective-drag-handle", "hoverable"},
...
function QMQuestManagerWindow.ShowAddNoteDialog(quest)
    ...
    ThemeEngine.OnThemeChanged(mod, function()
        if addNoteWindow ~= nil and addNoteWindow.valid then
            addNoteWindow.styles = ThemeEngine.GetStyles()
        end
    end)
...
function QMQuestManagerWindow._buildQuestForm(quest)
    local function stackedRow(labelText, control, width)
        return gui.Panel {
    ...
    local titleField = gui.Input {
        ...
        edit = function(element)
            if element.text ~= quest:GetTitle() then
                updateNetworkDoc(function() quest:SetTitle(element.text) end)
            end
        end
    }
    ...
    local descriptionField = gui.Input {
        ...
        edit = function(element)
            if quest:GetDescription() ~= element.text then
                updateNetworkDoc(function() quest:SetDescription(element.text) end)
            end
        end
    }
    ...
    local questGiverField = gui.Input {
        ...
        edit = function(element)
            if quest:GetQuestGiver() ~= element.text then
                updateNetworkDoc(function() quest:SetQuestGiver(element.text) end)
            end
        end
    }
    ...
    local locationField = gui.Input {
        ...
        edit = function(element)
            if quest:GetLocation() ~= element.text then
                updateNetworkDoc(function() quest:SetLocation(element.text) end)
            end
        end
    }
    ...
    local rewardsField = gui.Input {
        ...
        edit = function(element)
            if quest:GetRewards() ~= element.text then
                updateNetworkDoc(function() quest:SetRewards(element.text) end)
            end
        end
    }
    ...
    local categoryDropdown = gui.Dropdown {
        ...
        change = function(element)
            if quest:GetCategory() ~= element.idChosen then
                updateNetworkDoc(function() quest:SetCategory(element.idChosen) end)
            end
        end
    }
    ...
    local priorityDropdown = gui.Dropdown {
        ...
        change = function(element)
            if element.idChosen ~= quest:GetPriority() then
                updateNetworkDoc(function() quest:SetPriority(element.idChosen) end)
            end
        end
    }
    ...
    local statusDropdown = gui.Dropdown {
        ...
        change = function(element)
            if element.idChosen ~= quest:GetStatus() then
                updateNetworkDoc(function() quest:SetStatus(element.idChosen) end)
            end
        end
    }
    ...
    local rewardsClaimedCheckbox = gui.Check {
        ...
        change = function(element)
            if element.value ~= quest:GetRewardsClaimed() then
                updateNetworkDoc(function() quest:SetRewardsClaimed(element.value) end)
            end
        end
    }
    ...
    return gui.Panel {
        ...
        tmargin = 12,
        children = {
            stackedRow("Quest Title:", titleField),
            visibleToPlayersCheckbox,
            stackedRow("Description:", descriptionField),
            ...
            gui.Panel {
                ...
                halign = "left",
                children = {
                    stackedRow("Category:", categoryDropdown, "33%"),
                    stackedRow("Priority:", priorityDropdown, "33%"),
                    stackedRow("Status:", statusDropdown, "33%"),
                }
            },
            stackedRow("Quest Giver:", questGiverField),
            stackedRow("Location:", locationField),
            stackedRow("Rewards:", rewardsField),
